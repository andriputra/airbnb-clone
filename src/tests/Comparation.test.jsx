...
describe("Comparation Component", () => {
  ...
  test("renders the comparison table", () => {
    ...
    data.forEach((item) => {
      ...
      expect(airbnbIcon).toHaveClass(
        item.airbnbStatus === faCheck ? "text-green-500" : "text-red-500"
      );

      const competitorsIcon = screen.getByText((content, element) => {
        return element.tagName.toLowerCase() === "svg" && content === "";
      });
      expect(competitorsIcon).toHaveClass(
        item.competitorsStatus === faCheck ? "text-green-500" : "text-red-500"
      );
    });
  });
});
